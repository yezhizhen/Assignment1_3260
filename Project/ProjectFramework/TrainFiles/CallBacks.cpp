...
void deleteTrainCB(Fl_Widget*, TrainWindow* tw)
{
	if(tw->world.trainnum > 0)
		tw->world.trainnum--;
	tw->damageMe();

}

void addTrainCB(Fl_Widget*, TrainWindow* tw)
{
	//limit of new car numbers
	if(tw->world.trainnum < MAX_CAR_NUM)
		tw->world.trainnum++;
	tw->damageMe();
}
...
void runButtonCB(TrainWindow* tw)
{
	...
	if (tw->runButton->value()) {	// only advance time if appropriate.. i.e., when the run is pressed and 1/30 seconds past
...
void changeSmoothness(Fl_Widget*, TrainWindow* tw)
{
	//linear
	if(tw->splineBrowser->value()==1)
	{
		//C0 continuous
		tw->world.smoothness = 0;
	...
	{
		//C1 continuous
		tw->world.smoothness = 1;
		//if not running, just reset
		if (tw->runButton->value() == 0){
